...
if st.button("Generate with AI"):
    if user_prompt.strip():
        ...
        if result:
            # Set flag to indicate content was generated by AI
            ...
            # Display the summary right after generation
            st.markdown("---")
...
if st.session_state.get("ai_generated", False):
    ...
    employment_type = ", ".join(st.session_state["values"]["employment_type"]) if st.session_state["values"]["employment_type"] else "Not specified"
    
    # Format salary information
    salary_info = f"{st.session_state['values']['salary_amount']} {st.session_state['values']['salary_currency']} {st.session_state['values']['salary_period']}"
    if st.session_state['values']['salary_amount'] == 0:
        salary_info = "Not specified"
    
    # Format workplace information
    workplace_info = st.session_state["values"]["workplace_type"]
    if st.session_state["values"]["workplace_location"]:
        workplace_info += f" in {st.session_state['values']['workplace_location']}"
    
    # Create a summary table
    summary_data = {
        "Field": ["Job Title", "Employment Type", "Workplace", "Salary", "Education", 
                 "Job Description", "Employee Benefits", "Required Skills"],
        "Value": [
            st.session_state["values"]["job_title"],
            employment_type,
            workplace_info,
            salary_info,
            st.session_state["values"]["education"] or "Not specified",
...
def ai_badge():
    """Create an AI edited badge if content was generated by AI"""
    if st.session_state.get("ai_generated", False):
        return " ✨ Edited by AI"
    return ""

# Then modify the form inputs to include badges
# --- Job Ad Form Inputs ---
st.text_input(
    f"Job Title{ai_badge()}", 
    st.session_state["values"].get("job_title", ""),
    key="job_title_input"
)
st.session_state["values"]["job_title"] = st.session_state["job_title_input"]

st.multiselect(
    f"Employment Type{ai_badge()}",
    ["full-time", "part-time", "internship", "trade licence", "agreement-based"],
    default=st.session_state["values"].get("employment_type", []),
    key="employment_type_input"
)
st.session_state["values"]["employment_type"] = st.session_state["employment_type_input"]

st.selectbox(
    f"Workplace Type{ai_badge()}",
    ...
    key="workplace_type_input"
)
st.session_state["values"]["workplace_type"] = st.session_state["workplace_type_input"]

st.text_input(
    f"Workplace Location{ai_badge()}", 
    st.session_state["values"].get("workplace_location", ""),
    key="workplace_location_input"
)
st.session_state["values"]["workplace_location"] = st.session_state["workplace_location_input"]
...
col1.number_input(
    f"Salary Amount{ai_badge()}", 
    value=salary_value,
    key="salary_amount_input"
)
st.session_state["values"]["salary_amount"] = st.session_state["salary_amount_input"]

col2.selectbox(
    f"Currency{ai_badge()}", 
    ["EUR", "CZK", "HUF"], 
    index=["EUR", "CZK", "HUF"].index(st.session_state["values"]["salary_currency"]),
    key="salary_currency_input"
)
st.session_state["values"]["salary_currency"] = st.session_state["salary_currency_input"]

col3.selectbox(
    f"Salary Period{ai_badge()}", 
    ["per month", "per hour"], 
    index=["per month", "per hour"].index(st.session_state["values"]["salary_period"]),
    key="salary_period_input"
)
st.session_state["values"]["salary_period"] = st.session_state["salary_period_input"]

st.selectbox(
    f"Education Attained{ai_badge()}",
    [
...
st.session_state["values"]["education"] = st.session_state["education_input"]
...
st.text_area(
    f"Generated Job Description{ai_badge()}", 
    value=st.session_state["values"].get("job_description_html", ""), 
    height=180,
    key="job_description_input"
)
st.session_state["values"]["job_description_html"] = st.session_state["job_description_input"]

st.subheader("🎁 Employee Benefits")
st.text_area(
    f"Generated Benefits{ai_badge()}", 
    value=st.session_state["values"].get("employee_benefits_html", ""), 
    height=150,
    key="employee_benefits_input"
)
st.session_state["values"]["employee_benefits_html"] = st.session_state["employee_benefits_input"]

st.subheader("🧠 Personality & Skills")
st.text_area(
    f"Generated Skills{ai_badge()}", 
    value=st.session_state["values"].get("personality_prerequisites_and_skills_html", ""), 
    height=150,
    key="personality_prerequisites_input"
)
st.session_state["values"]["personality_prerequisites_and_skills_html"] = st.session_state["personality_prerequisites_input"]
